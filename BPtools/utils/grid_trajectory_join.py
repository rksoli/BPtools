...
class DataProcess:
    ...
    def extract_data(self, traj1, traj2, grid1, grid2, gridTn, trajsTn, i):
        if trajsTn[i].shape[0] > self.delta_T12:
            index = trajsTn.shape[0] - self.delta_T12
            traj1.append(trajsTn[i][index:index+self.delta_T1, 0:2])
            traj1.append(trajsTn[i][index + self.delta_T1:index+self.delta_T12, 0:2])
            g1 = np.array(gridTn[i][index:index+self.delta_T1:self.grid_dT])
            grid1.append(g1)
            grid2.append(np.array(gridTn[i][index + self.delta_T1:index+self.delta_T12:self.grid_dT])[:, 7:23,63:191])
            return True
        return False
